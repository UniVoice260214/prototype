...
def make_pipeline(
    *,
...
) -> tuple[TranslationPipeline, list[SpeechSegment], list[tuple[str, str, SpeechSegment]]]:
    ...
    return pipeline, segments, captions


@pytest.mark.asyncio
async def test_fast_stt_finals_keep_sequence_order() -> None:
    pipeline, segments, captions = make_pipeline()
...
async def test_one_segment_failure_does_not_stop_next_segment() -> None:
    ...
    pipeline, segments, captions = make_pipeline(translator=translator)
    ...
    assert [segment.sequence for segment in segments] == [1, 2, 3]
    assert [text for _, text, _ in captions] == ["en:good.", "en:after."]
