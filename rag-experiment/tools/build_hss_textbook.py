...
tools/hss_content_part{1..5}.py 의 CHAPTERS(20개 장)를 읽어
...
OUT_PATH = ROOT / "data" / "raw" / "Humanities_Social_Sciences" / "인문사회_입문_교재.pdf"
BOOK_TITLE = "인문사회 입문"
BOOK_SUBTITLE = "학문의 기초부터 철학, 심리, 사회, 경제, 정치, 법, 문화, 역사, 언어, 한국 어문, 문헌정보까지"
...
def load_chapters() -> list[dict]:
    chapters: list[dict] = []
    for i in range(1, 6):
        path = Path(__file__).resolve().parent / f"hss_content_part{i}.py"
        spec = importlib.util.spec_from_file_location(f"hss_content_part{i}", path)
...
def title_page(st: dict) -> list:
    return [
        ...
        Spacer(1, 9 * cm),
        p("연구방법 · 철학 · 윤리 · 심리 · 사회 · 경제 · 정치 · 법 · 문화 · 역사 · 언어 · 한국 어문 · 문헌정보", st["title_foot"]),
        p("입문자를 위한 인문사회 종합 개념 교재", st["title_foot"]),
    ]
...
def build() -> None:
    ...
    doc = BookDoc(
        ...
        title=BOOK_TITLE, author="HSS Textbook Generator",
    )
